...
`Realtime Outputs` is a canonical element of `engine_runtime_realtime` inside `L1. Runtime Kernel`. It exists as one explicit part of the engine stack and does not transfer its ownership class into another crate or level.

## Data Model

Interactive progression outputs and realtime diagnostics emitted by realtime runs.
