...
`Thermal Field` is a canonical element of `engine_field` inside `L2. Critical Simulation Families`. It exists as one explicit part of the engine stack and does not transfer its ownership class into another crate or level.

## Data Model

Heat transfer, ignition thresholds, drying, and combustion-adjacent thermal progression.
