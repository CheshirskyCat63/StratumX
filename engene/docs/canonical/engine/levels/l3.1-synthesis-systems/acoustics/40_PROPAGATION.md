...
`Propagation` is a canonical element of `engine_acoustics` inside `L3.1. Synthesis Systems`. It exists as one explicit part of the engine stack and does not transfer its ownership class into another crate or level.

## Data Model

Sound propagation structures and propagation outputs over spatial partitions.
